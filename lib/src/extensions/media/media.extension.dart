...
class MediaExtension extends BaseExtension {
  MediaExtension(final Client client, final String domain)
      : super(client, to: Node(name: postmasterName, domain: '$postmasterDomain.$domain'));

  Future<Command> getUploadToken({bool secure = false}) {
    return processCommand(createGetCommand(buildResourceQuery(UriTemplates.mediaUpload, {'secure': secure})));
  }

  Future<Command> refreshMedia(id) {
    return processCommand(createGetCommand(buildUri(UriTemplates.refreshMedia, [id])));
  }
}
